...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'WETDOGRUNSWILD'
    answer = big_letters([(3, 1, 4),  # not a string
                          "We'll Escape This place someday",  # WET
                          (10, 'Ok what is ThiSSS?', 10),  # not a string
                          [],  # not a string
                          ['Huh'],  # not a string
                          "DO you know what Grade he's in?",  # DOG
                          ['OOPS'],  # not a string
                          'Rare to see a UnicorN in these partS.',  # RUNS
                          'Where Is he Looking? the back Den?' #WILD
                          ])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j]) is str:
                if  sequence_of_sequences[j][k].isupper():
                    string = string + str(sequence_of_sequences[j][k])
    return string
